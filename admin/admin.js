  ...
  card.innerHTML = `
    ...
    ${order.status === 'unpaid' ? `
      <div class="card-actions">
        <button class="btn-card-secondary" onclick="printInvoice('${order.id}')" style="grid-column: 1 / 2">Print Invoice</button>
        <button class="btn-card-secondary" onclick="printPhotoSlip('${order.id}')" style="background:#e8f8f5; border-color:#a3e4d7; color:#16a085; grid-column: 2 / 3;">Photo Slip</button>
        <button class="btn-card-danger" onclick="cancelOrder('${order.id}')" style="grid-column: 1 / -1">Cancel Order</button>
      </div>
    ` : order.status === 'paid' ? `
      <div class="card-actions">
      ...
      snap.docs.forEach((d, idx) => {
        ...
        rowsHtml += `
          <tr>
            ...
            <td>${b.phone || '-'}</td>
            <td>${b.city || '-'}</td>
            <td><span style="background:rgba(39,174,96,0.2); color:#27ae60; padding:4px 8px; border-radius:4px; font-weight:700; font-size:0.9rem;">Paid ₹500</span><br><span style="font-size:0.75rem; color:#888;">${b.paymentId || 'PAY_SLOT_CONFIRMED'}</span></td>
            <td>
