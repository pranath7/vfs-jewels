...
INPUT_DIR = r'C:\Users\91636\.gemini\antigravity-ide\scratch\cleaned_necklaces'
...
CATEGORY = 'necklaces'
META = '18K Gold Plated CZ Necklace'
BADGE = 'New Arrival'
PRICE = 499
WHOLESALE_PRICE = 499
MRP = 749
...
def save_to_firestore_product(product):
    ...
    res = requests.patch(url, data=body, headers={"Content-Type": "application/json"}, timeout=15)
    if res.status_code == 200:
        return True
    else:
        print(f"  ⚠ Firestore product update error: {res.status_code} - {res.text[:200]}")
        return False
...
def save_to_firestore_stock(product_id, stock_count=25):
    ...
    if res.status_code == 200:
        ...
        print(f"  ⚠ Firestore stock update error: {res.status_code} - {res.text[:200]}")
        return False

def main():
    files = sorted([f for f in os.listdir(INPUT_DIR) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.webp'))])
    print(f"[FILES] Found {len(files)} cleaned images in {INPUT_DIR}")
    ...
    start_id = max_id + 1
    print(f"Starting product ID: {start_id}")
    
    new_products = []
    ratings = ['4.7', '4.8', '4.9', '5.0', '4.8', '4.9', '4.7', '4.8']
    review_counts = [14, 18, 22, 16, 20, 25, 12, 19]
    
    for i, file_name in enumerate(files):
        p_id = start_id + i
        num = str(i + 1).zfill(2)
        sku = f"SN-N{str(p_id).zfill(3)}"
        ...
        product = {
            "id": p_id,
            "sku": sku,
            "name": f"VFS Gold Plated CZ Necklace #{num}",
            "cat": CATEGORY,
            "meta": META,
            "price": PRICE,
        ...
        existing_urls[f"vfsneck01/{file_name}"] = img_url
        time.sleep(0.3)
        
    merged = existing_products + new_products
    with open(PRODUCTS_FILE, 'w', encoding='utf-8') as f:
        json.dump(merged, f, indent=2, ensure_ascii=False)
    print(f"\n[DONE] Updated {PRODUCTS_FILE}. Total products: {len(merged)}")
    
    with open(URLS_FILE, 'w', encoding='utf-8') as f:
        json.dump(existing_urls, f, indent=2, ensure_ascii=False)
    print(f"[DONE] Updated {URLS_FILE}.")
