...
window.getPaymentPauseNotice = function() {
  return window.VFS_CONFIG?.storeStatus?.pauseNotice || "We are temporarily pausing checkout & online payments for maintenance and inventory updates. You can still browse our collections. We will resume accepting orders shortly!";
};
  ...
  if (checkoutBtn) {
    if (isPaused) {
      ...
      if (checkoutBtn.innerHTML.includes('Orders Temporarily Paused')) {
        checkoutBtn.innerHTML = 'Proceed to Checkout • <span id="vfsCartBtnTotal">₹0</span>';
      }
    }
  }
};
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (config.firebase && config.firebase.apiKey && !config.firebase.apiKey.startsWith("YOUR_")) {
        ...
        // Sync catalog from Firestore by merging with catalog
        const dbProducts = await window.VFS_DB.getProducts();
        if (dbProducts && dbProducts.length > 0) {
          const map = new Map();
          (window.VFS_PRODUCTS_CACHE && window.VFS_PRODUCTS_CACHE.length > 0 ? window.VFS_PRODUCTS_CACHE : PRODUCTS).forEach(p => map.set(String(p.id), p));
...
window._vfsProductsListenerActive = false;
function setupRealtimeProductsListener() {
  if (!window.db || window._vfsProductsListenerActive) return;
  window._vfsProductsListenerActive = true;
  try {
    window.db.collection('products').onSnapshot(snapshot => {
      if (!snapshot) return;
      let hasChanges = false;
      const prodMap = new Map();
      (window.VFS_PRODUCTS_CACHE && window.VFS_PRODUCTS_CACHE.length > 0 ? window.VFS_PRODUCTS_CACHE : PRODUCTS).forEach(p => prodMap.set(String(p.id), p));
      ...
      if (hasChanges) {
        ...
        console.log("🔥 Real-time sync: Catalog updated from Firestore.");
        if (typeof renderProducts === 'function') renderProducts(null);
        if (typeof renderProductShelves === 'function') renderProductShelves();
        ...
        if (window.currentPdpProductId) {
          const curIdStr = String(window.currentPdpProductId);
          const modal = document.getElementById('productModal') || document.getElementById('pdpOverlay');
          if (modal && (modal.classList.contains('active') || modal.style.display !== 'none')) {
            const updatedP = window.VFS_PRODUCTS_CACHE.find(p => String(p.id) === curIdStr);
            if (updatedP && typeof openPDP === 'function') {
              openPDP(updatedP.id);
            }
          }
        }
      ...
      console.warn("Firestore products snapshot listener note:", err);
    });

    // Real-time stock updates
    window.db.collection('product_stock').onSnapshot(snapshot => {
      if (!snapshot) return;
      ...
      snapshot.docChanges().forEach(change => {
        ...
        if (!isNaN(newStock)) {
          const numId = !isNaN(Number(pId)) ? Number(pId) : pId;
          window.VFS_STOCK_CACHE[pId] = newStock;
          window.VFS_STOCK_CACHE[numId] = newStock;
          const p = (window.VFS_PRODUCTS_CACHE || PRODUCTS).find(x => String(x.id) === pId);
      ...
      console.warn("Firestore stock snapshot listener note:", err);
    });
  } catch(e) {
    console.warn("Error attaching realtime products listener:", e);
  }
}
...
function renderProducts(filter) {
  ...
  categories.forEach(cat => {
    ...
    <div class="p-img">
      <img src="${clOpt(p.img, 380)}" alt="${p.name}" loading="lazy" decoding="async" onerror="this.onerror=null;this.src='${p.img || ''}'" >
      ${quickActionHtml}
    </div>
                <div class="p-info">
  ...
  if (window.isPaymentsPaused && window.isPaymentsPaused()) {
    ...
    if (shoppingMode === 'wholesale') {
      ...
      if (grandTotal < minOrder) {
        ...
        if (moqWarning) {
          moqWarning.textContent = `Wholesale MOQ is ₹1,000. Add ${fmt(remaining)} more to proceed.`;
...
function openCheckout() {
  ...
  // MOV ₹1,000 applies to wholesale orders only
  if (!isDemoOnly && shoppingMode === 'wholesale' && cartSubtotal < 1000) {
    toast(`Wholesale minimum order is ₹1,000. Add ${fmt(1000 - cartSubtotal)} more to proceed.`);
    return;
  }
...
function openPDP(id) {
  ...
  (function setupSharing() {
    ...
    const instaLink = `https://www.instagram.com/vfs_jewels/`;
    ...
    $('#pdpShareLinks').innerHTML = `
      ...
      <a href="${instaLink}" class="share-icon share-instagram" title="Follow & Share on Instagram" target="_blank" rel="noopener">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="20" height="20" x="2" y="2" rx="5"/><circle cx="12" cy="12" r="5"/><path d="M17.5 6.5h.01"/></svg>
      </a>
...
async function initApp() {
  ...
  // Build stock cache from merged catalog
  PRODUCTS.forEach(p => {
    const s = (p.stock !== undefined && p.stock !== null) ? Number(p.stock) : 0;
    window.VFS_STOCK_CACHE[p.id] = s;
    window.VFS_STOCK_CACHE[String(p.id)] = s;
  ...
  try {
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        const paidUnlockFee = numAmt || 1000;

        if (!pPhone || pPhone.length !== 10) {
          pPhone = savedPhone;
        }

        // Calculate instant new wallet balance
        let currentBal = 0;
        try {
          if (window.VFS_DB && window.VFS_DB.getCustomerWalletBalance) {
            currentBal = await window.VFS_DB.getCustomerWalletBalance(pPhone);
          } else {
            currentBal = parseFloat(localStorage.getItem('vfs_wallet_balance') || '0');
          }
        } catch(e) {
          currentBal = parseFloat(localStorage.getItem('vfs_wallet_balance') || '0');
        }
        ...
        localStorage.setItem('vfs_user_mode', 'wholesale');
        localStorage.setItem('vfs_shopping_mode', 'wholesale');
        localStorage.setItem('vfs_customer_phone', pPhone);
        localStorage.setItem('vfs_customer_name', pName);
        localStorage.setItem('vfs_wallet_balance', newBal.toString());
        document.documentElement.setAttribute('data-shopping-mode', 'wholesale');
        if (typeof wholesaleUnlocked !== 'undefined') wholesaleUnlocked = true;

        // 2. Save to Firestore wholesale_users & wallet_credits for real-time Admin Panel sync
        try {
          if (window.VFS_CLOUD_ACTIVE && window.db) {
            ...
            await window.db.collection('wallet_credits').doc(pPhone).set({ phone: pPhone, balance: newBal, updatedAt: Date.now() }, { merge: true });
            console.log('✅ Instant Firestore customer & wallet balance sync completed:', pPhone, newBal);
          }
        } catch (dbErr) {
          console.error("Firestore customer save error:", dbErr);
        }

        // 3. Save to localStorage wholesale_users map
        try {
          const mockUsers = JSON.parse(localStorage.getItem('vfs_wholesale_users') || '{}');
          mockUsers[pPhone] = docPayload;
          mockUsers['phone_' + pPhone] = docPayload;
          mockUsers['91' + pPhone] = docPayload;
          localStorage.setItem('vfs_wholesale_users', JSON.stringify(mockUsers));
        } catch(lsErr) {
          console.warn('Localstorage wholesale_users note:', lsErr);
        }

        // 4. Save to VFS_DB helper if available
        ...
        // 5. Trigger backend API for WhatsApp welcome & server-side sync
        try {
          fetch('https://www.vfsjewels.store/api/send-wholesale-welcome', {
            ...
            console.log('✅ Wholesale Welcome WhatsApp & DB sync response:', res);
          }).catch(err => console.warn('WhatsApp welcome notification warning:', err));
        } catch(e) {}

        // 6. Instant UI re-render
        if (typeof updateWalletDisplay === 'function') updateWalletDisplay(newBal);
        if (typeof updateCounts === 'function') updateCounts();
        ...
        window.completeWholesaleUnlock();
        if (typeof toast === 'function') {
          toast(`🎉 ₹${paidUnlockFee.toLocaleString('en-IN')} Received! Wholesale Unlocked & ₹${paidUnlockFee.toLocaleString('en-IN')} credited to Wallet (+91 ${pPhone})`);
        }
        alert(`🎉 Payment successful! Your Wholesale Portal is unlocked and ₹${paidUnlockFee.toLocaleString('en-IN')} has been credited to your wallet (+91 ${pPhone})!`);
...
// 7B. Razorpay ₹1 Slot Booking SDK Trigger
  ...
  try {
    ...
    try {
      const res = await fetch('/api/create-razorpay-order', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ amount: 1, currency: 'INR', receipt: 'slot_' + Date.now() })
      });
      if (res.ok) {
        const data = await res.json();
        if (data.id) orderId = data.id;
    ...
    const options = {
      ...
      currency: "INR",
      name: "VFS JEWELS",
      description: "Live 8:30 PM Video Session Slot Booking (₹1)",
      image: "https://res.cloudinary.com/cwx4zame/image/upload/v1783183760/ze9xek1cled8puy6vfex.png",
      order_id: orderId || undefined,
      handler: async function (response) {
...
function initAllMasterModalListeners() {
  ...
  // K. Live Slot Booking Initialization
  initLiveSlotBooking();
}
...
function initLiveSlotBooking() {
  ...
  const todayStr = new Date().toISOString().split('T')[0];
  let isSlotEnabledToday = false;
  let currentBookedCount = 0;

  const renderSlotUI = () => {
    containers.forEach(c => {
      if (!isSlotEnabledToday) {
        c.innerHTML = `
          <div style="text-align:center; padding:24px 16px; background:rgba(231, 76, 60, 0.08); border-radius:12px; border:1px solid rgba(231, 76, 60, 0.4); margin-bottom:16px;">
            <span style="background:#e74c3c; color:#fff; font-size:0.9rem; font-weight:800; padding:6px 14px; border-radius:4px; text-transform:uppercase; letter-spacing:0.5px; display:inline-block; margin-bottom:12px;">🔒 BOOKING CURRENTLY CLOSED</span>
            <h3 style="font-size:1.4rem; color:var(--text-primary); margin-bottom:8px; font-weight:800; font-family:var(--font-heading);">Today's 8:30 PM Session Is Closed</h3>
            <p style="font-size:1.1rem; color:var(--text-muted); line-height:1.5; margin:0;">
              Live Video Preview slots reset automatically at 12:00 AM midnight every day. Booking opens once enabled by our host for today's 8:30 PM session. Please check back later!
            </p>
          </div>`;
        return;
      }
      ...
      const totalSlots = 24;
      const availableSlots = Math.max(0, totalSlots - currentBookedCount);
      ...
      if (form) {
        ...
        form.onsubmit = async function(e) {
          ...
          // Open Razorpay Slot Booking Payment
  ...
  // 1. Initial REST API Fetch for Slot Settings & Bookings Count
  (async () => {
    try {
      // Fetch slot settings
      const settingsRes = await fetch('https://firestore.googleapis.com/v1/projects/vfs-jewellery/databases/(default)/documents/settings/live_slot_settings');
      if (settingsRes.ok) {
        const sData = await settingsRes.json();
        const fields = sData.fields || {};
        const settingDate = fields.date ? fields.date.stringValue : '';
        const enabled = fields.enabled ? fields.enabled.booleanValue : false;
        // Enabled ONLY if stored date is today AND enabled is true
        isSlotEnabledToday = (settingDate === todayStr) && enabled;
      }
      ...
      console.warn('Initial REST slot fetch note:', e);
      renderSlotUI();
    }
  })();

  // 2. Real-time Firestore SDK Listeners
  const setupRealtimeWebListener = () => {
    if (window.db) {
      try {
        // Real-time listener for Settings
        if (window.webSlotSettingsUnsub) window.webSlotSettingsUnsub();
        window.webSlotSettingsUnsub = window.db.collection('settings').doc('live_slot_settings')
          .onSnapshot(doc => {
            if (doc.exists) {
              const data = doc.data();
              isSlotEnabledToday = (data.date === todayStr) && !!data.enabled;
              renderSlotUI();
            }
          });
